...
nix_err nix_store_get_uri(nix_c_context *, Store *store, char *dest,
                          unsigned int n);

// returns: owned Store*
StorePath *nix_store_parse_path(nix_c_context *, Store *store,
                                const char *path);
...
nix_err nix_store_build(nix_c_context *, Store *store, StorePath *path,
