...
def nix_store_init() -> None:
    ctx().err_check(lib.nix_libstore_init)
...
class Store:
    def __init__(
        ...
        self._store = ffi.gc(
            ctx().null_check(lib.nix_store_open, url_c, params_c), lib.nix_store_unref
        )

    def get_uri(self) -> str:
        dest = ffi.new("char[256]")
        ctx().err_check(lib.nix_store_get_uri, self._store, dest, len(dest))
        return ffi.string(dest).decode()

    def get_version(self) -> str:
        dest = ffi.new("char[256]")
        ctx().err_check(lib.nix_store_get_version, self._store, dest, len(dest))
        return ffi.string(dest).decode()

    def parse_path(self, path: str) -> StorePath:
        path_ct = ffi.new("char[]", path.encode())
        sp = ctx().null_check(lib.nix_store_parse_path, self._store, path_ct)
    ...
    def build(self, path: StorePath | str) -> dict[str, str]:
        ...
        ctx().err_check(lib.nix_store_build, self._store, path._path, iter_callback)
        return res
